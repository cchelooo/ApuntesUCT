...
export function CatalogFilterSidebar() {
  ...
  const [checkedUniversities, setCheckedUniversities] = useState<Set<string>>(
    new Set()
  );
  const [checkedCareers, setCheckedCareers] = useState<Set<string>>(new Set());
  const [checkedProfessors, setCheckedProfessors] = useState<Set<string>>(
    new Set()
  );

  const hasActiveFilters =
    checkedUniversities.size > 0 ||
    checkedCareers.size > 0 ||
    checkedProfessors.size > 0;
  ...
  return (
    ...
    >
      ...
      <fieldset className="border-b border-catalog-line px-4 py-4">
        <legend className="font-display text-sm text-catalog-ink">
          Universidad
        </legend>
